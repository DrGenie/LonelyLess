    ...
    <body>
      ...
      <script>
        ...
        const benefitData = savedScenarios.map(s => {
          ...
          const qalyPerParticipant = ${QALY_SCENARIOS_VALUES['moderate']};
...
function exportToPDF() {
  ...
  savedScenarios.forEach((scenario, index) => {
    ...
    doc.text(`Virtual: ${scenario.virtualCheck ? 'Yes' : 'No'}`, 10, startY + 40);
    doc.text(`Hybrid: ${scenario.hybridCheck ? 'Yes' : 'No'}`, 10, startY + 45);
    doc.text(`2-Hour Interaction: ${scenario.twoHCheck ? 'Yes' : 'No'}`, 10, startY + 50);
    doc.text(`4-Hour Interaction: ${scenario.fourHCheck ? 'Yes' : 'No'}`, 10, startY + 55);
